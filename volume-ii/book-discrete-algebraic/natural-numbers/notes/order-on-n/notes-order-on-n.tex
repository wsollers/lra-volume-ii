...
\begin{description}
  \item[Exposition.]
  Strict inequality fails exactly when no additive witness in $P$ carries
  $a$ to $b$. In that case, every nontrivial additive step from $a$ misses
  $b$.
...
\[
\operatorname{Transitive}(\leq_P,P).
\]
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
\operatorname{Antisymmetric}(\leq_P,P).
\]
\end{remark*}
...
\begin{remark*}[Failure modes]
\begin{description}
  \item[Exposition.] The named branch below records the failed antisymmetry
  pattern.
  \item[\(\operatorname{Antisymmetric}\) fails.] Antisymmetry would fail only
  if two distinct elements were comparable in both directions:
  \[
  \exists a,b \in P\;\bigl(a\neq b \land a\le b \land b\le a\bigr).
  \]
...
\begin{remark*}[Predicate reading]
\[
\operatorname{OrderPreservingRightAddition}(P,\leq_P,+_P).
\]
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
\operatorname{OrderPreservingLeftAddition}(P,\leq_P,+_P).
\]
\end{remark*}
...
\begin{theorembox}{Theorem (Addition Preserves Order)}
...
\]
\hyperref[prf:addition-preserves-order]{Go to proof.}
\end{theorem}
\end{theorembox}
...
&\text{Let } (P,S,1) \text{ be a Peano system.}\\
&\forall a,b,c \in P\;
\bigl((a \le b \Longrightarrow a+c \le b+c)
\land
(a \le b \Longrightarrow c+a \le c+b)\bigr).
\end{aligned}
\]
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
\operatorname{OrderCompatible}(P,\leq_P,+_P).
\]
\end{remark*}

\begin{remark*}[Interpretation]
The right and left compatibility laws together say that adding a common
element preserves order no matter which side carries the common summand.
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
\operatorname{OrderEmbedding}(P,<_{P},P,<_{P},x \mapsto k\cdot x).
\]
\end{remark*}
...
\begin{remark*}[Predicate reading]
\[
\forall a,b \in P\;(a<b \Longleftrightarrow S(a) \le b).
\]
\end{remark*}
...
\begin{theorembox}{Theorem (Natural-Number Order Is Total)}
\begin{theorem}[Natural-Number Order Is Total]
\label{thm:natural-number-order-is-total}
Let $(P,S,1)$ be a Peano system. For every $a,b \in P$,
\[
a \le b
\qquad\text{or}\qquad
...
\hyperref[prf:natural-number-order-is-total]{Go to proof.}
\end{theorem}
\end{theorembox}
...
\forall a,b \in P\;(a \le b \lor b \le a).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{TotalRelation}(\leq_P,P).
\]
\end{remark*}

\begin{remark*}[Interpretation]
Trichotomy says exactly one of $a<b$, $a=b$, or $b<a$ holds. Since the
non-strict order allows either strict comparison or equality, trichotomy
immediately yields total comparability for $\le$.
\end{remark*}
