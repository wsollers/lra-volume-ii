...
\end{definition}
\end{definitionbox}
\LeanFormalizes{def:addition-on-peano-system}{lra-lean}{LRA.VolumeII.NaturalNumbers.Operations.Addition}{NAddition}{pending}
...
\begin{description}
  ...
  Once the recursive map $f_m$ is fixed, every incorrect candidate for $m+n$
  is simply a value of $P$ different from $f_m(n)$.
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:addition-well-defined-on-peano-system}{lra-lean}{LRA.VolumeII.NaturalNumbers.Operations.Addition}{NAdditionWellDefined}{pending}
...
\[
\operatorname{WellDefinedBinaryOperation}(+_P,P).
\]
\end{remark*}
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:addition-with-one}{lra-lean}{LRA.VolumeII.NaturalNumbers.Operations.Addition}{NAdditionWithOne}{pending}
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:addition-successor-on-right}{lra-lean}{LRA.VolumeII.NaturalNumbers.Operations.Addition}{NAdditionSuccessorOnRight}{pending}
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:addition-associative}{lra-lean}{LRA.VolumeII.NaturalNumbers.Operations.Addition}{NAdditionIsAssociative}{pending}
...
\end{theorem}
\end{theorembox}
\LeanFormalizes{thm:addition-commutative}{lra-lean}{LRA.VolumeII.NaturalNumbers.Operations.Addition}{NAdditionIsCommutative}{pending}
...
\begin{description}
  ...
  pattern for left cancellation.
  \item[\(\operatorname{LeftCancellation}\) fails.] Left cancellation would
  fail only if distinct inputs became equal after adding the same element on the
  left:
  \[
  \exists a,b,c \in P\;\bigl(b \neq c \land a+b=a+c\bigr).
  \]
...
\begin{description}
  \item[Exposition.] The named branch below records the contrapositive failure
  pattern for right cancellation.
  \item[\(\operatorname{RightCancellation}\) fails.] Right cancellation would
  fail only if distinct inputs became equal after adding the same element on the
  right:
  \[
  \exists a,b,c \in P\;\bigl(b \neq c \land b+a=c+a\bigr).
  \]
  In predicate-reading form this is recorded by the negation of
  \[
  \operatorname{RightCancellation}(P,+_P).
...
\begin{theorembox}{Theorem (Cancellation for Addition)}
\begin{theorem}[Cancellation for Addition]
\label{thm:addition-cancellation}
...
\]
\hyperref[prf:addition-cancellation]{Go to proof.}
\end{theorem}
\end{theorembox}
...
&\text{Let } (P,S,1) \text{ be a Peano system.}\\
&\forall a,b,c \in P\;
\bigl((a+b=a+c \Longrightarrow b=c) \land
(b+a=c+a \Longrightarrow b=c)\bigr).
\end{aligned}
\]
\end{remark*}
...
\begin{remark*}[Interpretation]
The left and right cancellation laws together say that adding a common natural
number never destroys equality information, regardless of which side contains
the common summand.
\end{remark*}
