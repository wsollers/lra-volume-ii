\subsection*{The Extended Successor}

\begin{definitionbox}{Definition (Successor on \texorpdfstring{$\mathbb{W}$}{W})}
\begin{definition}[Successor on $\mathbb{W}$]
\label{def:successor-on-w}
Define $S_{\mathbb{W}}:\mathbb{W}\to\mathbb{W}$ by
\[
S_{\mathbb{W}}(0)=1
\qquad\text{and}\qquad
S_{\mathbb{W}}(n)=S_{\mathbb{N}}(n)\quad(n\in\mathbb{N}).
\]
\end{definition}
\end{definitionbox}

\begin{remark*}[Standard quantified statement]
\[
\begin{aligned}
&S_{\mathbb{W}}:\mathbb{W}\to\mathbb{W},\\
&S_{\mathbb{W}}(0)=1,\\
&\forall n\in\mathbb{N}\;
  \bigl(S_{\mathbb{W}}(n)=S_{\mathbb{N}}(n)\bigr).
\end{aligned}
\]
\end{remark*}

\begin{remark*}[Interpretation]
The definition inserts $0$ immediately before the positive chain. After the
first step, successor is exactly the successor already defined on
$\mathbb{N}$.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:whole-numbers]{Whole Numbers}
  \item \hyperref[def:peano-system]{Peano System}
\end{itemize}
\end{dependencies}

\begin{theorembox}{Theorem (Successor on \texorpdfstring{$\mathbb{W}$}{W} Is Well-Defined)}
\begin{theorem}[Successor on $\mathbb{W}$ Is Well-Defined]
\label{thm:successor-on-w-well-defined}
The rule defining $S_{\mathbb{W}}$ determines a function
$S_{\mathbb{W}}:\mathbb{W}\to\mathbb{W}$.
\hyperref[prf:successor-on-w-well-defined]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
S_{\mathbb{W}}\text{ is a well-defined function from }\mathbb{W}
\text{ to }\mathbb{W}.
\]
\end{remark*}

\begin{remark*}[Interpretation]
The cases are exclusive because $0\notin\mathbb{N}$, and the positive case
lands in $\mathbb{N}\subseteq\mathbb{W}$.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:whole-numbers]{Whole Numbers}
  \item \hyperref[def:successor-on-w]{Successor on $\mathbb{W}$}
\end{itemize}
\end{dependencies}

\begin{theorembox}{Theorem (\texorpdfstring{$0$}{0} Is Not a Successor in \texorpdfstring{$\mathbb{W}$}{W})}
\begin{theorem}[$0$ Is Not a Successor in $\mathbb{W}$]
\label{thm:zero-is-not-successor-in-w}
There is no $a\in\mathbb{W}$ such that $S_{\mathbb{W}}(a)=0$.
\hyperref[prf:zero-is-not-successor-in-w]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
\neg\exists a\in\mathbb{W}\;\bigl(S_{\mathbb{W}}(a)=0\bigr).
\]
\end{remark*}

\begin{remark*}[Interpretation]
The new element $0$ is the first element of the extended chain. It is placed
before $1$, not obtained by applying successor to some earlier element.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:successor-on-w]{Successor on $\mathbb{W}$}
  \item \hyperref[ax:peano-base-not-successor]{$1$ Is Not a Successor}
\end{itemize}
\end{dependencies}

\begin{theorembox}{Theorem (Every Nonzero Element of \texorpdfstring{$\mathbb{W}$}{W} Is a Successor)}
\begin{theorem}[Every Nonzero Element of $\mathbb{W}$ Is a Successor]
\label{thm:every-nonzero-w-is-successor}
If $a\in\mathbb{W}$ and $a\neq 0$, then there exists
$b\in\mathbb{W}$ such that $a=S_{\mathbb{W}}(b)$.
\hyperref[prf:every-nonzero-w-is-successor]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
\forall a\in\mathbb{W}\;
\bigl(a\neq 0 \Longrightarrow
\exists b\in\mathbb{W}\;(a=S_{\mathbb{W}}(b))\bigr).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{SuccessorCoversNonbase}(S_{\mathbb{W}},0,\mathbb{W}).
\]
\end{remark*}

\begin{remark*}[Interpretation]
The element $1$ is the successor of $0$, and every positive natural number
beyond $1$ remains a successor in the original Peano chain.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:whole-numbers]{Whole Numbers}
  \item \hyperref[def:successor-on-w]{Successor on $\mathbb{W}$}
  \item \hyperref[ax:peano-induction]{Induction Axiom}
\end{itemize}
\end{dependencies}

\begin{theorembox}{Theorem (Successor on \texorpdfstring{$\mathbb{W}$}{W} Is Injective)}
\begin{theorem}[Successor on $\mathbb{W}$ Is Injective]
\label{thm:successor-on-w-injective}
For all $a,b\in\mathbb{W}$,
\[
S_{\mathbb{W}}(a)=S_{\mathbb{W}}(b)\Longrightarrow a=b.
\]
\hyperref[prf:successor-on-w-injective]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
\forall a,b\in\mathbb{W}\;
\bigl(S_{\mathbb{W}}(a)=S_{\mathbb{W}}(b)\Longrightarrow a=b\bigr).
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{Injective}(S_{\mathbb{W}},\mathbb{W},\mathbb{W}).
\]
\end{remark*}

\begin{remark*}[Interpretation]
The extended successor still never merges two inputs. The only new case is the
comparison between $0$ and a positive natural number, and that case is ruled
out by the original non-return condition for the first element.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:successor-on-w]{Successor on $\mathbb{W}$}
  \item \hyperref[ax:peano-successor-injective]{Injectivity of Successor}
  \item \hyperref[ax:peano-base-not-successor]{$1$ Is Not a Successor}
\end{itemize}
\end{dependencies}

\begin{theorembox}{Theorem (\texorpdfstring{$(\mathbb{W},0,S_{\mathbb{W}})$}{(W,0,S_W)} Satisfies the Successor Properties)}
\begin{theorem}[$(\mathbb{W},0,S_{\mathbb{W}})$ Satisfies the Successor Properties]
\label{thm:w-successor-system-properties}
The triple $(\mathbb{W},0,S_{\mathbb{W}})$ satisfies the successor-side
properties:
\[
0\in\mathbb{W},\qquad
S_{\mathbb{W}}:\mathbb{W}\to\mathbb{W},
\]
\[
\neg\exists a\in\mathbb{W}\;(S_{\mathbb{W}}(a)=0),
\qquad
\forall a,b\in\mathbb{W}\;
(S_{\mathbb{W}}(a)=S_{\mathbb{W}}(b)\Longrightarrow a=b).
\]
\hyperref[prf:w-successor-system-properties]{Go to proof.}
\end{theorem}
\end{theorembox}

\begin{remark*}[Standard quantified statement]
\[
\begin{gathered}
0\in\mathbb{W},\qquad
S_{\mathbb{W}}:\mathbb{W}\to\mathbb{W},\\
\neg\exists a\in\mathbb{W}\;(S_{\mathbb{W}}(a)=0),\\
\forall a,b\in\mathbb{W}\;
\bigl(S_{\mathbb{W}}(a)=S_{\mathbb{W}}(b)\Longrightarrow a=b\bigr).
\end{gathered}
\]
\end{remark*}

\begin{remark*}[Predicate reading]
\[
\operatorname{SuccessorSystemProperties}(\mathbb{W},0,S_{\mathbb{W}}).
\]
\end{remark*}

\begin{remark*}[Interpretation]
Adjoining $0$ shifts the base of the successor chain from $1$ to $0$. The
resulting successor structure has a base element, closes under successor, has
no predecessor for the base, and keeps successor injective.
\end{remark*}

\begin{dependencies}
\begin{itemize}
  \item \hyperref[def:whole-numbers]{Whole Numbers}
  \item \hyperref[def:successor-on-w]{Successor on $\mathbb{W}$}
  \item \hyperref[thm:successor-on-w-well-defined]{Successor on $\mathbb{W}$ Is Well-Defined}
  \item \hyperref[thm:zero-is-not-successor-in-w]{$0$ Is Not a Successor in $\mathbb{W}$}
  \item \hyperref[thm:successor-on-w-injective]{Successor on $\mathbb{W}$ Is Injective}
\end{itemize}
\end{dependencies}
